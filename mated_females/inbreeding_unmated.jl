...
function mating_non_inbred(females,males,pmat)
...
function mating(females,males,female_detail,male_detail,previous_mated_females,pmat)
    # Takes arrays of male & female genotypes, and produces vector of mated females
    # Given one male and one female, pmat is the probability that they find each other and mate (pmat does not account for fecundity)
    ...
    for i = 1:n
        if females[i] != sum(female_detail[:,i]) || males[i] != sum(male_detail[:,i])
            println("Error code 2.1")
        end
    end
    ...
    # One entry in the vector of mated_females[:,1] for each unordered pair of genotypes, and one for females which don't find a mate.
    # We count ordered pairs of genotypes in the following way, as given by the function allele_comb above.
    # Consider n genotypes called 1,2,3,...,n. Order pairs as (1,1),...,(1,n),(2,1),...,(2,n),...,(n,1),...,(n,n).
    # Including females which do not find a mate, there are (1/2)n(n+1)+1 entries pf mated_females[:,1]. The final entry is non-mated females.
    # Second column for sibmated females
    mated_females = zeros(Int,n^2 + 1,2);
    #println("size of mated females vector is $(size(mated_females))")
    ...
    for i = 1:n
        v = rand(Multinomial(females[i],prob_vec));
        for j = 1:n
            mated_females[genotype_position(i,j,n),1] += v[j];
        end
        mated_females[end,1] += v[end];
    end

    prob_sibmating = zeros(n,n);
    for i = 1:n, j=1:n
        if females[i] >0 && males[j] > 0
            for kk = 1:n^2
                if previous_mated_females[kk] >0
                    prob_sibmating[i,j] += (female_detail[kk,i]/females[i]) * (male_detail[kk,j]/males[j]) * 1/previous_mated_females[kk];
                    if isnan(prob_sibmating[i,j]) == true
                        println("kk = $kk")
                        println("i = $i, j = $j")
                        println("female_detail = $(female_detail[kk,i])")
                        println("females = $(females[i])")
                        println("male_detail = $(male_detail[kk,j])")
                        println("males = $(males[j])")
                        println("previous_mated_females = $(previous_mated_females[kk])")
                        break
                    end
                end
            end
    ...
    for i = 1:n, j = 1:n
        kk = genotype_position(i,j,n)
        mated_females[kk,2] = rand(Binomial(mated_females[kk,1],prob_sibmating[i,j]));
        mated_females[kk,1] += -mated_females[kk,2];
    end
...
function offspring_production(mated_genotype_pair,no_females,totfem,Rm,K,fitness_vector,sibmating_cost,gamete_array)
    ...
    if fitness == 0
        ...
        exptd_pop = 2*fitness*(1-sibmating_cost)*Rm*no_females/(1+2*totfem*(Rm-1)/K);
...
function fixedmig_TSR_unmated(s,hD,hN,σ,δ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width::Int,height::Int,pmig,pmat,preexisting,T,test_threshold)
    ...
    mated_females = zeros(width,height,n^2,2); # Throw away unmated females to save memory

    # Each timestep, we'll keep track of which fe/male genotypes are produced by which mated genotype pairs (ie. mothers' and fathers' genotypes)
    female_genotype_offspring = zeros(width,height,n^2,n);
    male_genotype_offspring = zeros(width,height,n^2,n);
    ...
    for t = 1:T-1
        ...
        if t == 1
            for i=1:width, j=1:height
                mated_females[i,j,:,1] = mating_non_inbred(females[i,j,:],males[i,j,:],pmat)[1:end-1];
            end
        else
            for i=1:width, j=1:height
                mated_females[i,j,:,:] = mating(females[i,j,:],males[i,j,:],female_genotype_offspring[i,j,:,:],male_genotype_offspring[i,j,:,:],mated_females[i,j,:,1]+mated_females[i,j,:,2],pmat)[1:end-1 , :];
            end
        end
        ### Throw away females which did not find a mate.
        ###mated_females = mated_females[1:end-1];
        ...
        for i = 1:width, j = 1:height
            ...
            males[i,j,:] = zeros(Int,n);
            for kk = 1:n^2
                (ff,mm) = offspring_production(kk,mated_females[i,j,kk,1],totfem,Rm,K_perdeme,fitness_vector,0,gamete_array);
                female_genotype_offspring[i,j,kk,:] += ff;
                male_genotype_offspring[i,j,kk,:] += mm;
                females[i,j,:] += ff;
                males[i,j,:] += mm;
                (ff,mm) = offspring_production(kk,mated_females[i,j,kk,2],totfem,Rm,K_perdeme,fitness_vector,δ,gamete_array);
                female_genotype_offspring[i,j,kk,:] += ff;
                male_genotype_offspring[i,j,kk,:] += mm;
                females[i,j,:] += ff;
                males[i,j,:] += mm;
        ...
        # Unmated individuals migrate
        for ii = 1:n
            females[:,:,ii] = square_deme_migration(females[:,:,ii],pmig,Gauss);
            males[:,:,ii] = square_deme_migration(males[:,:,ii],pmig,Gauss);
        end
        ...
        if sum(popsize[3,:,:,t+1]) == 0
            println("Population zero at t = $t")
            break
        end
    end
