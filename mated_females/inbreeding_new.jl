...
function fixedmig_TSR_mated(s,hD,hN,σ,δ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width::Int,height::Int,pmig,pmat,preexisting,T,test_threshold)
    ...
    for t = 1:T-1
        ...
        # Mated females migrate
        for ii = 1:n^2, jj = 1:2
            mated_females[:,:,ii,jj] = square_deme_migration(mated_females[:,:,ii,jj],pmig,Gauss);
        end
        # Now for females and males in next generation
